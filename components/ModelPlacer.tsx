...
export function ModelPlacer({
  ...
  const commitPlacement = useCallback(
    (clientX: number, clientY: number) => {
      ...
      const hits = raycastSceneForSurfaces(raycaster.current, sceneRoot);
      const hit = pickSurfaceHit(hits);
      if (!hit?.face) {
        onPlacementMissed?.();
        return;
      }

      const pose = poseFromSurfaceHit(hit);
      onPlaced({
